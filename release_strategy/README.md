...
This folder defines exactly what to publish from `ghostchat` so others can run it quickly without your private data.
...
- Python 3.10+
- Node.js 18+
- `openclaw-main` placed next to `ghostchat` (external dependency, not bundled)
...
```powershell
powershell -ExecutionPolicy Bypass -File .\ghostchat\release_strategy\create_public_bundle.ps1 -OutputDir .\dist\ghostrelay-public
```

## Publish checklist
1. Build bundle with `create_public_bundle.ps1`.
2. Confirm `ghostchat/settings.json` is not present.
